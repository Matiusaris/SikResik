...
claimBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        alert('Promo berhasil diklaim! Screenshot pesan ini dan tunjukkan ke Admin saat pemesanan.');
...
//select cleaner
function selectCleaner(name, img, rating, badge) {
    const bookingSection = document.getElementById('booking-section');
    
    // Gunakan flex agar wrapper di dalamnya bisa sejajar
    bookingSection.style.display = 'flex'; 
    
    const infoPanel = document.getElementById('selected-cleaner-info');
    infoPanel.innerHTML = `
        <img src="${img}" class="mini-img">
        <div class="mini-info">
            <p style="font-size: 0.8rem; font-weight: bold; color: var(--secondary);">Cleaner Terpilih:</p>
            <h3>${name}</h3>
            <p><i class="fas fa-star" style="color: #ffcc00;"></i> ${rating} | ${badge}</p>
        </div>
    `;
...
// Paket Lainnya
function showServiceDetail(name, img, price, desc, baImg, cleaner) {
    // Tampilkan panel
    const panel = document.getElementById('service-detail-panel');
    panel.style.display = 'block';

    // Update konten detail
    ...
    document.getElementById('det-ba').src = baImg;
    document.getElementById('det-cleaner').innerText = cleaner;
...
function showServiceDetail(name, img, price, desc, baImg, cleaner) {
    ...
    const cleanerBadge = document.getElementById('det-cleaner');
    cleanerBadge.innerText = cleaner;
    
    cleanerBadge.onclick = function() {
        selectCleaner(cleaner, 'img/images.png', '4.9', 'Pilihan Terbaik');
    };
